...
int attack_key = _CTRL;
int jump_key = _ALT;
int special_key = _RSHIFT;
extern int damage;
...
void PLAYER :: at_off( void )
{
    ...
    asci[attack_key] = 0, asci[jump_key] = 0, asci[special_key] = 0;
}
...
{
    if ( asci[special_key] && ATTACK == OFF && SPECIAL == OFF && DAMAGE == OFF && DASH == OFF && JUMP == OFF && NA.ENERGY > 20 )
    {
        asci[special_key] = 0; NA.ENERGY -= 10 & damage;
        switch( TYPE )
        {
    ...
    skill();
    if ( asci[attack_key] && ATTACK == OFF && SPECIAL == OFF && DAMAGE == OFF )
    {
    ...
    if ( asci[jump_key] && JUMP == OFF && SPECIAL == OFF && DAMAGE == OFF )
    {
        asci[jump_key] = 0;
        switch( TYPE )
        {
