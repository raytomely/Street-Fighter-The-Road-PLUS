...
void stage_init( void ) //Ïiœ‹a 1¤å : ˆa“w, 2¤å : ”a·q·a¡ ñ´áˆq
{
    switch( ROUND )
    {
        case 1:
...
int main(int argc, char *argv[])
{
    ...
    for ( i = 0; i < 400; i++ ) {if(read_keyboard()) break; FRAME( 10 );}
    for ( i = 100; i >= 0; i-- ) fade_run( i ), FRAME( 10 );
    if ( SOUND2 ) playMidiData( ".\\ARM\\TITLE.ARM", 0 );
    clear_page( 1 ); copy_page_x( 1 ); fade_run( 100 ); RESTORE_PALETTE();
    read_pcx( ".\\ARG\\TITLE.ARG", 1 );
    gul( 88, 258, "Ported By Raytomely", 10, 1 );
    copy_page_x( 1 );

    while( !scan[_ENTER] )
    {processKeyboardInput(); sleep();} delay(100);
    ...
    while( !asci[_ESC] )
    {
        ...
        if ( START == 0 )
        {
            ...
            for ( PYOSI_Y = 0; PYOSI_Y < 350; PYOSI_Y++ )
            {
                ...
                ei--;if ( EN[ei].PY + EY[EN[ei].ACT[EN[ei].act]] + EN[ei].sy[EN[ei].act] == PYOSI_Y ) EN[ei].jang_move();
            }
        ...
        processKeyboardInput(); sleep();
        //if ( scan[_A] ) CLEAR = 1;
        if ( asci[_P] ) { asci[_P] = 0; while( !asci[_P] ) { if ( scan[_O] ) break; processKeyboardInput(); sleep(); } asci[_P] = 0; asci[_O] = 0; }
    }
    if ( ROUND == 7 ) gl_start();
    end();
